...
class AfandyTravelApp {
  constructor() {
      ...
      this.handleOutsideClick = null // إضافة متغير لتخزين معالج الأحداث
      this.heroInterval = null // إضافة متغير للـ slideshow
  ...
  // إضافة دالة initScrollAnimations المفقودة
  initScrollAnimations() {
      // تحقق من وجود العناصر قبل إضافة الرسوم المتحركة
      const elementsToAnimate = document.querySelectorAll('.animate-on-scroll, .fade-in-up, .slide-in-left, .slide-in-right')
      ...
      // إضافة رسوم متحركة للتمرير المتوازي (Parallax)
      this.initParallaxEffects()
  }

  // إضافة دالة التأثيرات المتوازية
  initParallaxEffects() {
  ...
  initHeroSlideshow() {
      ...
      // Improved parallax for hero images
      let ticking = false
      const updateParallax = () => {
      ...
      testimonialCards.forEach((card) => {
          card.addEventListener("mouseenter", () => {
              ...
              if (stars) {
                  ...
                  stars.innerHTML = ""
                  Array.from(starText).forEach((star, index) => {
  ...
  showNotification(message, type = "info") {
      ...
      notification.style.cssText = `
          ...
          top: 2rem;
          right: 2rem;
          background: ${type === "success" ? "var(--afandy-teal)" : type === "error" ? "#ef4444" : "var(--gray-800)"};
          color: white;
          padding: 1rem 1.5rem;
...
// Header scroll effects - Enhanced version
document.addEventListener('DOMContentLoaded', () => {
...
// Background Slider - Enhanced version
document.addEventListener('DOMContentLoaded', () => {
...
// Testimonial Carousel - Complete Module
const TestimonialCarouselModule = (function() {
  ...
  function toggleTestimonialPlay(id) {
      ...
      if (!videoElement || !thumbElement || !playOverlay || !closeBtn || !testimonial) return
      ...
      Object.keys(videoPlayingStates).forEach(videoId => {
          if (videoId != id && videoPlayingStates[videoId]) {
              ...
              if (otherVideo && otherThumb && otherOverlay && otherCloseBtn) {
  ...
  function restartTestimonialAutoSlide() {
      ...
      setTimeout(startTestimonialAutoSlide, 100)
  }
...
// دوال عرض الصور والرسوم المتحركة
